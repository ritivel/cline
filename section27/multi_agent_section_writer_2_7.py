"""Multi-Agent System for Writing ICH Module 5 Section 2.7 LaTeX Documents

This system uses agents to write LaTeX sections for each subsection of 2.7,
...
LATEX_EXAMPLE = r"""
\section{2.7.1 Summary of Biopharmaceutic Studies and Associated Analytical Methods}
\label{sec:2_7_1}

\subsection{2.7.1.1 Background and Overview}
\label{subsec:2_7_1_1}

This section provides an overall view of the formulation development process, the in vitro and in vivo dosage form performance, and the general approach used in developing the bioavailability (BA), comparative BA, bioequivalence (BE), and in vitro dissolution profile database. Detailed biopharmaceutic studies are presented in \modref{5.3.1.1}.

\subsection{2.7.1.2 Summary of Results of Individual Studies}
\label{subsec:2_7_1_2}

A tabular listing of all biopharmaceutic studies is provided, together with narrative descriptions of relevant features and outcomes of each study. Key study results are detailed in \modref{5.3.1.2}.
"""
...
class SectionWritingState(TypedDict):
    ...
    section_id: str  # e.g., "2.7.1", "2.7.2", etc.
    ...
    other_sections: Dict[str, str]  # Other 2.7 sections for cross-referencing {section_id: title}
...
def load_section_guidance(section_id: str, base_path: str = "section2.7") -> str:
    """Load guidance text for a specific section.

    Args:
        section_id: Section ID (e.g., "2.7.1", "2.7.2", "2.7.4.1")
        base_path: Base path to section2.7 folder
    ...
    if not filepath.exists():
        ...
        # e.g., if 2.7.4.1.txt doesn't exist, look in 2.7.4.txt
...
def load_preamble(base_path: str = "section2.7") -> str:
    """Load preamble from 2.7.txt.
...
def get_all_section_info(base_path: str = "section2.7") -> Dict[str, Dict[str, str]]:
    """Get information about all sections in 2.7.
    ...
    # Section titles mapping - all 27 sections of 2.7
    section_titles = {
...
def get_related_sections(section_id: str, all_sections: Dict[str, Dict[str, str]]) -> Dict[str, Dict[str, str]]:
    ...
    parts = section_id.split('.')
    if len(parts) > 3:
        # e.g., 2.7.4.1 -> also reference 2.7.4
        parent_id = '.'.join(parts[:-1])
...
def find_relevant_papers(section_id: str, papers_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Map section 2.7.x to corresponding 5.3.x sections
    # Base mappings for main sections
    base_section_mapping = {
        "2.7.1": ["5.3.1", "5.3.1.1", "5.3.1.2", "5.3.1.3", "5.3.1.4"],  # Biopharmaceutics
        "2.7.2": ["5.3.2", "5.3.2.1", "5.3.2.2", "5.3.2.3", "5.3.3", "5.3.3.1",
                  "5.3.3.2", "5.3.3.3", "5.3.3.4", "5.3.3.5", "5.3.4", "5.3.4.1", "5.3.4.2"],  # Clinical Pharmacology
    ...
    # Handle nested sections - map to specific 5.3.x sections
    nested_section_mapping = {
        ...
        "2.7.2.2": ["5.3.2", "5.3.2.1", "5.3.2.2", "5.3.2.3", "5.3.3", "5.3.3.1", "5.3.3.2", "5.3.3.3", "5.3.3.4", "5.3.3.5", "5.3.4", "5.3.4.1", "5.3.4.2"],
        "2.7.2.3": ["5.3.2", "5.3.2.1", "5.3.2.2", "5.3.2.3", "5.3.3", "5.3.3.1", "5.3.3.2", "5.3.3.3", "5.3.3.4", "5.3.3.5", "5.3.4", "5.3.4.1", "5.3.4.2"],
    ...
    if section_id in nested_section_mapping:
        ...
        # e.g., 2.7.4.1.1 -> use 2.7.4.1 mapping
        parts = section_id.split('.')
        if len(parts) > 3:
            parent_id = '.'.join(parts[:-1])
    ...
    # For 2.7.5 (References) and 2.7.6 (Synopses), include all papers
    if section_id in ["2.7.5", "2.7.6"]:
        for section_key, section_data in sections.items():
            section_papers = section_data.get("papers", [])
            for paper in section_papers:
                if not any(p.get("url") == paper.get("url") for p in relevant_papers):
...
def create_section_writer_agent(section_id: str, section_guidance: str,
    ...
    - Already-written LaTeX content from other 2.7 sections

    Args:
        section_id: Section ID (e.g., "2.7.1")
        section_guidance: Guidance text from .txt file
        relevant_papers: List of relevant papers
        related_sections: Dictionary of related 2.7 sections metadata
    ...
    sections_context = ""
    if related_sections:
        sections_context = "\n\nRELATED SECTIONS IN 2.7 FOR CROSS-REFERENCING:\n"
        sections_context += "=" * 60 + "\n"
        sections_context += "Cross-reference these sections using \\secref{2.7.X} command.\n"
...
- 1.1 Background and Overview
  • Key points: formulation development, BA/BE studies, analytical methods
  • Citations: [1] Smith2020, [3] Jones2021
  • Cross-ref: Links to Section 2.7.2

- 1.2 Summary of Results of Individual Studies
  • Key points: tabular listing, narrative descriptions, key findings
...
4. Fixing any LaTeX formatting issues, especially:
   - Avoid "Overfull \\hbox" errors by breaking long words/phrases
   - Ensure lines don't contain overly long unbreakable sequences
   - Use appropriate line breaks for long reference codes
   - Break long technical terms with hyphens
...
"""
    ...
    system_prompt = f"""You are an expert regulatory medical writer specializing in ICH Module 5 Section 2.7: Clinical Summary.
...
Note: Section 2.7 (Clinical Summary) is intended to provide a detailed, factual summarization of all clinical information. It focuses on factual observations, in contrast to Section 2.5 (Clinical Overview) which provides critical analysis and interpretation. The Clinical Summary typically ranges from 50-400 pages and presents data objectively.
...
• Section commands: Use the EXACT section number in the title: \\section{{{section_id} Title}}
  Example: \\section{{2.7.1 Summary of Biopharmaceutic Studies and Associated Analytical Methods}}
• Labels: \\label{{sec:{section_id.replace('.', '_')}}} immediately after section commands
• Cross-references to OTHER 2.7 sections: \\secref{{2.7.X}} (e.g., \\secref{{2.7.3}})
...
CRITICAL: AVOID OVERFULL HBOX ERRORS:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
To prevent "Overfull \\hbox" errors (text overflowing margins):

1. LONG WORDS AND TECHNICAL TERMS:
   • Break long technical terms or compound words using \\hyphen{{}} or manual breaks
   • For very long unbreakable terms, consider using \\textsc{{}} or rephrasing
   • Example: "pharmacokinetic-pharmacodynamic" instead of "pharmacokineticpharmacodynamic"

2. LONG REFERENCE STRINGS:
   • Break long reference codes: use "\\modref{{5.3.5.1}}" instead of long inline text
   • For URLs or long identifiers, use \\texttt{{}} or \\url{{}} from hyperref package
   • Break long study names: "Study\\hyphen{{}}ABC\\hyphen{{}}001" or rephrase

3. NUMBERS AND UNITS:
   • Add spaces or breaks: "$95\\%$ confidence interval" not "$95\\%confidenceinterval"
   • For long number ranges, use "to" instead of "--": "50 to 400 pages"

4. TABLES AND FIGURES:
   • Use tabularx or adjustbox for wide tables
   • For long table entries, use \\parbox{{}} or allow text wrapping
   • Ensure table widths don't exceed \\textwidth

5. SENTENCE STRUCTURE:
   • Keep sentences reasonably short (60-80 characters ideal)
   • Break long sentences at natural points (commas, conjunctions)
   • Use shorter alternative phrases when possible

6. WHEN UNAVOIDABLE:
   • For paragraphs with unavoidable overflow, wrap in: \\begin{{sloppypar}}...\\end{{sloppypar}}
   • Use \\linebreak or \\newline strategically (sparingly)
   • Consider using \\raggedright for certain paragraphs (use with caution)

7. TEST YOUR OUTPUT:
   • Ensure no line contains more than ~70-75 characters of visible text
   • Check for long unbreakable sequences (e.g., "pharmacokineticpharmacodynamic")
   • Verify tables fit within margins
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
A tabular listing of all biopharmaceutic studies is provided, together with narrative descriptions of relevant features and outcomes. Key study results are detailed in \\modref{{5.3.1.2}}.
...
7. Use \\secref{{2.7.X}} for cross-references to other 2.7 sections (e.g., \\secref{{2.7.3}})

QUALITY STANDARDS:
✓ Comprehensive coverage of all guidance topics
✓ At least 3-5 references to Module 5 sections (5.2 or 5.3.x)
✓ Cross-references to related 2.7 sections using \\secref{{2.7.X}}
✓ Factual, objective presentation (not interpretive analysis)
...
def planning_node(state: SectionWritingState) -> SectionWritingState:
    ...
    output_dir = state.get("output_dir", "section2.7_tex")
    ...
    # Get related sections within 2.7 (dependencies)
...
def validate_latex_quality(latex_content: str, section_id: str,
    ...
    # Check for potential overfull hbox issues - very long unbreakable words/sequences
    # Look for words longer than 25 characters without spaces or hyphens
    lines = latex_content.split('\n')
    for line_num, line in enumerate(lines[:200], 1):  # Check first 200 lines
        # Skip comments and commands
        if line.strip().startswith('%') or line.strip().startswith('\\'):
            continue
        # Find long unbreakable sequences (words without spaces/hyphens longer than 30 chars)
        long_words = re.findall(r'\b\w{30,}\b', line)
        if long_words:
            issues.append(f"Line {line_num}: Very long word(s) that may cause overfull hbox: {long_words[:3]}")
            suggestions.append(f"Break long words using hyphens or rephrase: {long_words[0]}")
            score -= 3
...
def load_written_section(section_id: str, output_dir: str = "section2.7_tex", max_chars: int = 3000) -> str:
...
def save_tex_file(section_id: str, latex_content: str, preamble: str = None,
                  output_dir: str = "section2.7_tex") -> str:
    """Save LaTeX content to a file.

    Args:
        section_id: Section ID (e.g., "2.7.1")
        latex_content: LaTeX content to save
        preamble: Optional preamble content (for 2.7.txt)
    ...
    # If this is 2.7.txt (preamble), create a special preamble file
    if section_id == "2.7" and preamble:
        full_content = f"""% Preamble for Section 2.7: Clinical Summary
...
def generate_main_tex(output_dir: str = "section2.7_tex",
    ...
    - Manual section numbering (preserves 2.7.x format)
    - All section includes
    - References integrated within section 2.7.5 only
    ...
    # Default sections if not provided - all 27 sections of 2.7
    if sections is None:
        sections = [
            "2.7.1", "2.7.1.1", "2.7.1.2", "2.7.1.3", "2.7.1.4",
            "2.7.2", "2.7.2.1", "2.7.2.2", "2.7.2.3", "2.7.2.4", "2.7.2.5",
            "2.7.3", "2.7.3.1", "2.7.3.2", "2.7.3.3", "2.7.3.4", "2.7.3.5", "2.7.3.6",
            "2.7.4", "2.7.4.1", "2.7.4.2", "2.7.4.3", "2.7.4.4", "2.7.4.5", "2.7.4.6", "2.7.4.7",
            "2.7.5", "2.7.6"
        ]
...
"""
    ...
    main_tex_content = f'''% =============================================================================
% ICH Module 5 Section 2.7: Clinical Summary
% Main LaTeX Document
% =============================================================================
...
\\hypersetup{{
    ...
    pdftitle={{ICH Module 5 Section 2.7 - Clinical Summary - {drug_name}}},
...
\\pagestyle{{fancy}}
\\fancyhf{{}}
\\fancyhead[L]{{\\small ICH Module 5 Section 2.7}}
...
% =============================================================================
% Use unnumbered sections but display manual numbers (preserves 2.7.x format)
\\setcounter{{secnumdepth}}{{0}}  % Disable automatic section numbering

% Custom reference commands
% Usage: \\modref{{5.3.1}} for Module 5 sections, \\secref{{2.7.1}} for internal sections
...
\\newcommand{{\\secref}}[1]{{(see Section #1)}}  % For internal 2.7.x section references
...
def write_all_sections(papers_json: str, sections: List[str] = None,
                       model: str = "openai:gpt-4o", output_dir: str = "section2.7_tex",
    ...
    """
    # Default sections to write - all 27 sections of 2.7
    if sections is None:
        sections = [
            "2.7.1", "2.7.1.1", "2.7.1.2", "2.7.1.3", "2.7.1.4",
            "2.7.2", "2.7.2.1", "2.7.2.2", "2.7.2.3", "2.7.2.4", "2.7.2.5",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Multi-Agent System for Writing ICH Module 5 Section 2.7 LaTeX Documents",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Write a single section
  python multi_agent_section_writer_2_7.py 2.7.1 papers.json

  # Write with specific model
  python multi_agent_section_writer_2_7.py 2.7.2 papers.json --model openai:gpt-4o

  # Write without refinement (faster)
  python multi_agent_section_writer_2_7.py 2.7.4 papers.json --no-refinement

  # Write all sections in dependency order
  python multi_agent_section_writer_2_7.py --all papers.json

  # Write specific sections in batch
  python multi_agent_section_writer_2_7.py --batch 2.7.1,2.7.1.1,2.7.1.2 papers.json
        """
    )
    parser.add_argument("section_id", nargs="?", help="Section ID to write (e.g., 2.7.1, 2.7.2)")
    ...
    parser.add_argument("--output-dir", "-o", default="section2.7_tex",
                        help="Output directory for .tex files (default: section2.7_tex)")
    ...
    parser.add_argument("--batch", "-b", type=str,
                        help="Comma-separated list of sections to write in batch (e.g., 2.7.1,2.7.1.1,2.7.1.2)")
    ...
    if not re.match(r'^2\.7(\.\d+)*$', section_id):
        print(f"Error: Invalid section ID format: {section_id}")
        print("Expected format: 2.7, 2.7.1, 2.7.2, etc.")
        sys.exit(1)
    ...
    if "output_tex" in result and result["output_tex"]:
        # Load preamble if this is section 2.7
        preamble = None
        if section_id == "2.7":
            try:
