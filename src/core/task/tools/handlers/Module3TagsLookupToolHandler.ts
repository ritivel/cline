...
const LOG_PREFIX = "[🔍 Module3TagsLookup]"

/**
 * Tool handler for looking up tags.md files from Module 3 sections
 * Used by section 2.3 preamble generation agent (Quality Overall Summary)
 */
export class Module3TagsLookupToolHandler implements IFullyManagedTool {
	readonly name = ClineDefaultTool.MODULE3_TAGS_LOOKUP

	constructor(private validator: ToolValidator) {}

	getDescription(block: ToolUse): string {
		const sectionNumber = (block.params as any).sectionNumber as string | undefined
		return `[${block.name} for module 3 section '${sectionNumber || "unknown"}']`
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!sectionNumber) {
			...
			showSystemNotification({
				subtitle: "Module3TagsLookup - Error",
		...
		try {
			...
			// Validate section number is from Module 3
			const module3 = EAC_NMRA_TEMPLATE.modules.find((m: { moduleNumber: number }) => m.moduleNumber === 3)
			if (!module3) {
				console.error(`${LOG_PREFIX} ❌ ERROR: Module 3 not found in template`)
				showSystemNotification({
					subtitle: "Module3TagsLookup - Error",
					message: "Module 3 not found in template",
				})
				return formatResponse.toolError("Module 3 not found in template")
			}

			// Find the section in Module 3
			const section = module3.sections[sectionNumber]
			if (!section) {
				console.error(`${LOG_PREFIX} ❌ ERROR: Section ${sectionNumber} not found in Module 3`)
				showSystemNotification({
					subtitle: "Module3TagsLookup - Error",
					message: `Section ${sectionNumber} not found`,
				})
				return formatResponse.toolError(`Section ${sectionNumber} not found in Module 3`)
			}
			...
			if (docsWithoutMmd > 0) {
				...
				showSystemNotification({
					subtitle: "Module3TagsLookup - Warning",
					message: `${docsWithoutMmd} docs missing .mmd files!`,
				})
			}

			showSystemNotification({
				subtitle: "Module3TagsLookup - Success",
	...
	private sectionToFolderPath(sectionNumber: string, dossierPath: string): string {
		...
		if (!(sectionNumber in SECTION_PARENT_MAP)) {
			...
			// For Module 3, handle special section IDs like "3.2.S.1.1" or "3.2.P.5.1"
