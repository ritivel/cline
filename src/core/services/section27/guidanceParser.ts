/**
 * Guidance file parser for Section 2.7 (Clinical Summary)
 * Loads and parses 2.7.x.txt guidance files from bundled resources
 */
...
export function getGuidanceFilesPath(extensionPath: string): string {
	return path.join(extensionPath, "resources", "section27Guidance")
}
...
export async function loadSectionGuidance(sectionId: string, extensionPath: string): Promise<string> {
	...
	try {
		...
		// e.g., if 2.7.6.1.txt doesn't exist, look in 2.7.6.txt
...
/**
 * Load the preamble from 2.7.txt
 */
export async function loadPreamble(extensionPath: string): Promise<string> {
	const guidancePath = getGuidanceFilesPath(extensionPath)
	const filepath = path.join(guidancePath, "2.7.txt")
...
export function parseGuidanceContent(content: string): GuidanceSection[] {
	...
	// Regex to match section headers like "2.7.1 Summary of Biopharmaceutic Studies"
...
/**
 * Load all Section 2.7 guidance files
 */
export async function loadAllSection27Guidance(extensionPath: string): Promise<Record<string, GuidanceSection>> {
	const guidancePath = getGuidanceFilesPath(extensionPath)
	const allGuidance: Record<string, GuidanceSection> = {}

	// Section files to load (main sections and their subsections)
	const sectionFiles = [
		"2.7.1.1.txt",
		"2.7.1.2.txt",
		"2.7.1.3.txt",
