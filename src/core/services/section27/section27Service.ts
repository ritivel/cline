/**
 * Section 2.7 (Clinical Summary) LaTeX Generation Service
...
import {
	getAllSectionInfo,
	getRelatedSections,
	loadAllSection27Guidance,
	loadSectionGuidance,
	topologicalSortSections,
} from "./guidanceParser"
import type { GenerateSection27Response, Paper, QualityReport, Section53PapersResult, SectionInfo } from "./types"
import { DEFAULT_QUALITY_THRESHOLDS, SECTION_27_TO_53_MAPPING, SECTION_TITLES } from "./types"
...
/**
 * Find papers relevant to a specific section 2.7.x
 */
function findRelevantPapers(sectionId: string, papersData: Section53PapersResult): Paper[] {
	...
	// Get mapped sections for this 2.7.x section
	let mappedSections = SECTION_27_TO_53_MAPPING[sectionId] || []
	...
	if (mappedSections.length === 0) {
		...
		if (parts.length > 3) {
			...
			mappedSections = SECTION_27_TO_53_MAPPING[parentId] || []
		}
	}

	// For 2.7.5 (References) and 2.7.6 (Synopses), include all papers
	if (sectionId === "2.7.5" || sectionId === "2.7.6") {
...
REMINDER: Use these reference formats:
  ...
  - \\secref{2.7.X} - for cross-references to other 2.7 sections
`
	context += "=".repeat(60) + "\n"
...
function buildSystemPrompt(
	...
	if (Object.keys(relatedSections).length > 0) {
		...
		sectionsContext += "Cross-reference these sections using \\secref{2.7.X} command.\n\n"
	...
	return `You are an expert regulatory medical writer specializing in ICH Module 5 Section 2.7: Clinical Summary.
...
7. Use \\secref{2.7.X} for cross-references to other 2.7 sections
...
function validateLatexQuality(latexContent: string, sectionId: string, expectedCitations: number = 3): QualityReport {
	...
	const sectionMatches = latexContent.match(sectionPattern) || []
...
Return ONLY the LaTeX code starting with the appropriate sectioning command.`
	...
	console.log(`[Section27Service] Generating LaTeX for section ${sectionId}...`)
	...
	try {
		...
		console.log(`[Section27Service] Generated ${latexContent.length} chars for ${sectionId}, score: ${qualityReport.score}`)

		return { latex: latexContent, qualityReport }
	} catch (error) {
		console.error(`[Section27Service] Error generating LaTeX for ${sectionId}:`, error)
		throw error
	}
}
...
/**
 * Main service function: Generate Section 2.7 LaTeX document
 */
export async function generateSection27(
	drugName: string,
	...
	try {
		...
		try {
			...
			console.log(`[Section27Service] Loaded ${papersData.summary?.totalUniquePapers || 0} papers from Section 5.3`)
		} catch (error) {
			return {
				success: false,
		...
		// Load all guidance
		const allGuidance = await loadAllSection27Guidance(extensionPath)
		const allSectionInfo = await getAllSectionInfo(extensionPath)

		// Get sections to generate (including 2.7 intro and all main sections)
		const sectionsToGenerate = ["2.7", "2.7.1", "2.7.2", "2.7.3", "2.7.4", "2.7.5", "2.7.6"]
		const sortedSections = topologicalSortSections(sectionsToGenerate)

		console.log(`[Section27Service] Generating ${sortedSections.length} sections in order: ${sortedSections.join(", ")}`)
		...
		for (const sectionId of sortedSections) {
			...
			if (!sectionGuidance) {
				console.warn(`[Section27Service] No guidance found for ${sectionId}, skipping`)
				continue
			}

			// Find relevant papers
			const relevantPapers = findRelevantPapers(sectionId, papersData)
			console.log(`[Section27Service] Found ${relevantPapers.length} relevant papers for ${sectionId}`)
		...
		console.log(`[Section27Service] LaTeX file written to: ${outputPath}`)
		...
		console.error(`[Section27Service] Generation failed:`, error)
		return {
			success: false,
