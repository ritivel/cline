...
// RenderToolApprovalRequest renders a tool approval request ("Ritivel wants to...")
...
// RenderToolExecution renders a completed tool execution ("Ritivel is ...ing")
...
func (tr *ToolRenderer) generateToolHeader(tool *types.ToolMessage, verbTense string) string {
	...
	case string(types.ToolTypeEditedExistingFile):
		...
		return fmt.Sprintf("### Ritivel %s `%s`", action, tool.Path)
	...
	case string(types.ToolTypeSearchFiles):
		if tool.Regex != "" && tool.Path != "" {
			...
			return fmt.Sprintf("### Ritivel %s `%s` in `%s`", action, tool.Regex, tool.Path)
		} else if tool.Regex != "" {
			if verbTense == "wants to" {
				action = "wants to search for"
			} else {
			...
			return fmt.Sprintf("### Ritivel %s `%s`", action, tool.Regex)
		} else {
			if verbTense == "wants to" {
				return "### Ritivel wants to search files"
			} else {
				return "### Ritivel is searching files"
			}
		}
	...
	case string(types.ToolTypeSummarizeTask):
		if verbTense == "wants to" {
			return "### Ritivel wants to condense the conversation"
		} else {
			return "### Ritivel condensed the conversation"
		}
	...
	default:
		...
		return fmt.Sprintf("### Ritivel %s tool: %s", verb, tool.Tool)
	}
}
...
func (tr *ToolRenderer) RenderCommandApprovalRequest(command string, autoApprovalConflict bool) string {
	...
	// Generate header
	header := fmt.Sprintf("### Ritivel wants to run `%s`", command)
	rendered := tr.renderMarkdown(header)
	output.WriteString(rendered)
	output.WriteString("\n")
...
func (tr *ToolRenderer) RenderCommandExecution(command string) string {
	command = strings.TrimSpace(command)
	header := fmt.Sprintf("### Ritivel is running `%s`", command)
	rendered := tr.renderMarkdown(header)
	return "\n" + rendered + "\n"
}
